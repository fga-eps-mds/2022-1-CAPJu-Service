...
describe.skip("etapas", () => {
  ...
  beforeAll(async () => {
    ...
    globalResponse = await supertest(app).post("/newStage").send({
      name: "name",
      time: "15",
      deleted: false,
    });
  ...
  test("testa o endpoint newStage se der errado", async () => {
    const response = await supertest(app).post("/newStage").send({
      name: "",
      time: "",
    });
  ...
  test("testa o endpoint newStage duplicado", async () => {
    await supertest(app).post("/newStage").send({
      name: "teste",
      time: "123",
    });
    const response2 = await supertest(app).post("/newStage").send({
      name: "teste",
      time: "123",
    });
  ...
  test("testa a remoção de uma etapa em /deleteStage", async () => {
    const response = await supertest(app).post("/deleteStage").send({
